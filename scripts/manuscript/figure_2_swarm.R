...
plot_degree <- ggplot(bds_means %>% filter(QR_Queen_Condition != "Queen"), aes(x = QR_Queen_Condition, y = Degree)) +
  geom_point(data = bds_means_of_means %>% filter(Queen == 1), aes(color = Trial, fill = Trial), size = 3, shape = 21, stroke = 0.2, position = position_dodge(width = 0.5), color = "black") +
  geom_beeswarm(aes(color = Trial), stroke = 0, size = 1, alpha = .2, method = "hex") +
  geom_point(data = bds_means_of_means %>% filter(Queen == 0), aes(color = Trial, fill = Trial), size = 3, shape = 21, stroke = 0.2, position = position_dodge(width = 0.5), color = "black") +
  scale_color_manual(
...
ggsave("figures/manuscript/figure_2.jpeg", plot = final_plot + theme(plot.margin = margin(1, 1, 1, 1, "cm")), width = 8.5, height = 6, dpi = 600)
