...
# Filter out queens
filtered_workers_data <- bds %>%
  filter(QR == TRUE)

# Calculate Ovary Index
filtered_workers_data$OvaryIndex <- filtered_workers_data$AverageOvaryWidth / filtered_workers_data$AverageWingLength

# Classify individuals by role
filtered_workers_data <- filtered_workers_data %>%
...
daytime_filtered_data <- filtered_workers_data %>%
...
combined_model_summary <- bind_rows(model_summary_list)
...
comprehensive_summary_table <- combined_model_summary %>%
...
significant_metrics <- comprehensive_summary_table %>%
  filter(p.value < 0.05) %>%
  select(metric, term, p.value)

print(significant_metrics)

# Save comprehensive summary table to CSV
write_csv(comprehensive_summary_table, "results/comprehensive_summary_table.csv")
