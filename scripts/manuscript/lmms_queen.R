...
# List of features to test
features_QR <- c("Degree", "move_perc", "mean_vel", "N90.Day4", "Initiation.Freq", "Close", "Between","clust", "OvaryIndex")

# Function to fit and summarize the model for each feature
fit_and_summarize_QR <- function(feature) {
  formula <- as.formula(paste(feature, "~ 1 + Queen + (1 | Trial) + (1 | Queen:Trial) + (1 | Day_Zeit) + (1 | Day_Zeit:Trial)"))
  model <- lmer(formula, data = bds_pooled)
  tidy_model <- tidy(model, effects = "fixed", conf.int = TRUE) %>%
    mutate(feature = feature, model_spec = paste(deparse(formula), collapse = " "))
  return(tidy_model)
...
# Fit and summarize models for each feature
tidy_results_list_QR <- lapply(features_QR, fit_and_summarize_QR)

# Combine tidy results into a single data frame
tidy_results_QR <- bind_rows(tidy_results_list_QR)

# Ensure no truncation in the output display
options(width = 200)

# Display tidy results
print(tidy_results_QR)

# Create a comprehensive summary table
comprehensive_summary_table_QR <- tidy_results_QR %>%
  select(feature, model_spec, term, estimate, std.error, statistic, p.value) %>%
  arrange(feature, term)

# Display comprehensive summary table
print(comprehensive_summary_table_QR)

# Get significant features
significant_features_QR <- comprehensive_summary_table_QR %>%
  filter(p.value < 0.05) %>%
  select(feature,term, p.value)

print(significant_features_QR)