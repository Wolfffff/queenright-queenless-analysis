...
for (i in 1:10) {
  for (j in 1:95) {
    if (file.exists(paste(prefixes[i], sprintf("%03d", j), "Reencoded_Head_to_Head_True_NWP.csv", sep = "_"))) {
      Deg <- read.csv(paste(prefixes[i], sprintf("%03d", j), "Reencoded_Head_to_Head_True_NWP.csv", sep = "_"))
    }
