...
for (i in 1:10) {
  for (j in 1:95) {
    if (file.exists(paste(prefixes[i], sprintf("%03d", j), "Reencoded_Head_to_Head_False_NWP.csv", sep = "_"))) {
      Deg <- read.csv(paste(prefixes[i], sprintf("%03d", j), "Reencoded_Head_to_Head_False_NWP.csv", sep = "_"))
    }
...
write.csv(TotalNWP, "BDSNWP_EfficFixed_Unpooled.csv")
```
