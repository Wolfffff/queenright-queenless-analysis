...
setwd("~/BigDataSheetDeDup_100924")
...
#prefixes = c("20221123_1543_AmericanoLatte_QR", "20221123_1543_AmericanoLatte_QL",  "MexHotChoc_QR_1216_1646", "MexHotChoc_QL_1216_1646", "20230213_1745_AlmdudlerGspritzt_C1", "20230213_1745_AlmdudlerGspritzt_C0")
Day = 1
Days <- list()
Start = 0
#Total Degree
for(i in 1:length(prefixes)){
  for(j in 0:95){
    if(file.exists(paste(prefixes[i],sprintf("%03d", j),"Reencoded","Head_to_Head_False_Centralities.csv",sep = "_"))) {
      Deg = read.csv(paste(prefixes[i],sprintf("%03d", j),"Reencoded","Head_to_Head_False_Centralities.csv",sep = "_"))
...
for(i in 1:length(prefixes)){
  for(j in 0:95){
    if(file.exists(paste(prefixes[i],sprintf("%03d", j),"Reencoded","Head_to_Head_True_Centralities.csv",sep = "_"))) {
      Deg = read.csv(paste(prefixes[i],sprintf("%03d", j),"Reencoded","Head_to_Head_True_Centralities.csv",sep = "_"))
      Deg$Node = sub('\\.0','',Deg$X)
      Deg$ExpDay = as.factor(floor((j/24)+1))
      Deg$Day = as.factor(floor(((j+17)/24)+1))
      Deg$Zeit = as.numeric((j+17)%%24)
...
for(i in 1:length(prefixes)){
  for(j in 0:95){
    if(file.exists(paste(prefixes[i],sprintf("%03d", j),"Reencoded","Head_to_Body_False_Centralities.csv",sep = "_"))) {
      Deg = read.csv(paste(prefixes[i],sprintf("%03d", j),"Reencoded","Head_to_Body_False_Centralities.csv",sep = "_"))
      Deg$Node = sub('\\.0','',Deg$X)
      Deg$ExpDay = as.factor(floor((j/24)+1))
      Deg$Day = as.factor(floor(((j+17)/24)+1))
      Deg$Zeit = as.numeric((j+17)%%24)
...
for(i in 1:length(prefixes)){
  for(j in 0:95){
    if(file.exists(paste(prefixes[i],sprintf("%03d", j),"Reencoded","8_8_3_1_10_interp_filtered.h5_TrackPresence.csv",sep = "_"))) {
      Deg = read.csv(paste(prefixes[i],sprintf("%03d", j),"Reencoded","8_8_3_1_10_interp_filtered.h5_TrackPresence.csv",sep = "_"))
...
for(i in 1:length(prefixes)){
  for(j in 0:95){
    if(file.exists(paste(prefixes[i],sprintf("%03d", j),"Reencoded","8_8_3_1_10_interp_filtered.h5_TrackPresenceAnt.csv",sep = "_"))) {
      Deg = read.csv(paste(prefixes[i],sprintf("%03d", j),"Reencoded","8_8_3_1_10_interp_filtered.h5_TrackPresenceAnt.csv",sep = "_"))
      Deg$track = sub('\\.0','',Deg$track)
      Deg$ExpDay = as.factor(floor((j/24)+1))
      Deg$Day = as.factor(floor(((j+17)/24)+1))
      Deg$Zeit = as.numeric((j+17)%%24)
...
for(i in 1:length(prefixes)){
  for(j in 0:95){
    if(file.exists(paste(prefixes[i],sprintf("%03d", j),"Reencoded","velstrackqueenstandard.csv",sep = "_")) | file.exists(paste(prefixes[i],sprintf("%03d", j),"Reencoded","velstrackqueen.csv",sep = "_"))){
      if(file.exists(paste(prefixes[i],sprintf("%03d", j),"Reencoded","velstrackqueenstandard.csv",sep = "_"))){
        Deg = read.csv(paste(prefixes[i],sprintf("%03d", j),"Reencoded","velstrackqueenstandard.csv",sep = "_"))
      }
      if(file.exists(paste(prefixes[i],sprintf("%03d", j),"Reencoded","velstrackqueen.csv",sep = "_"))){
        Deg = read.csv(paste(prefixes[i],sprintf("%03d", j),"Reencoded","velstrackqueen.csv",sep = "_"))
      }
      Deg$X = sub('\\.0','',Deg$X)
      Deg$ExpDay = as.factor(floor((j/24)+1))
      Deg$Day = as.factor(floor(((j+17)/24)+1))
...
for(i in 1:length(prefixes)){
  for(j in 0:95){
    if(file.exists(paste(prefixes[i],sprintf("%03d", j),"Reencoded","DegDir.csv",sep = "_"))) {
      Deg = read.csv(paste(prefixes[i],sprintf("%03d", j),"Reencoded","DegDir.csv",sep = "_"))
...
for(i in 1:length(prefixes)){
  for(j in 0:95){
    if(file.exists(paste(prefixes[i],sprintf("%03d", j),"Reencoded","Head_to_Head_False_Clustering.csv",sep = "_"))){
      Deg = read.csv(paste(prefixes[i],sprintf("%03d", j),"Reencoded","Head_to_Head_False_Clustering.csv",sep = "_"))
      Deg$Node = sub('\\.0','',Deg$X)
      Deg$ExpDay = as.factor(floor((j/24)+1))
      Deg$Day = as.factor(floor(((j+17)/24)+1))
      Deg$Zeit = as.numeric((j+17)%%24)
...
numerical_data$QR <- BDSMeansNoOv[complete.cases(BDSMeansNoOv), ]$QR
numerical_data$Trial <- BDSMeansNoOv[complete.cases(BDSMeansNoOv), ]$Trial
numerical_data$Queen <- BDSMeansNoOv[complete.cases(BDSMeansNoOv), ]$Queen
...
cutoff2 <- quantile(data_with_pca_qr$PC2, 1)
print(cutoff1)
print(cutoff2)
