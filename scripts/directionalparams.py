...
counts = pd.DataFrame({'count' : df_dir.groupby( [ "Origin interactor", "Destination interactor"] ).size()}).reset_index()
counts["log_count"] = np.log10(counts["count"]) + 1
counts["count_recip"] = 1/counts["count"]
G = nx.from_pandas_edgelist(counts, source='Origin interactor', target='Destination interactor', edge_attr=('count', 'log_count', 'count_recip'), create_using=nx.DiGraph())
...
param_df = pd.DataFrame({'params': ["Transitivity", "Average Clustering", "Sum", "Modularity"], 'values': [trans, clust, sumedg, modul]})
...
comb_df = pd.DataFrame({'InDegree':pd.Series(indegree_cent),'OutDegree':pd.Series(outdegree_cent),'Closeness':pd.Series(degree_clos),'Betweenness':pd.Series(degree_betw)})
