...
ggplot(TotalCent, aes(x = as.integer(Hour), y = Degree/20, group = ID)) +
...
df_for_stats$QR_Queen_Condition <- factor(df_for_stats$QR_Queen_Condition, levels = c("Queenright","Queenless"))

fm.null <- lmer(Degree ~ 1 + zeit + (1 | day) + (1 | source_colony), data = df_for_stats)
fm <- lmer(Degree ~ 1 + QR_Queen_Condition + zeit + (1 | day) + (1 | source_colony), data = df_for_stats)
