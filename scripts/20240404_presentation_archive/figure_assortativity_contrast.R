...
for (metric in unique_network_metrics) {
  ...
  # single_param_df$Col <- sub("_[^_]+$", "", single_param_df$ID)
  #  # get value before first _ in source_colony
  # single_param_df$source_colony <- sapply(strsplit(single_param_df$Col, "_"), `[`, 1)
  # single_param_df$source_colony <- factor(single_param_df$source_colony, levels = c("RooibosTea", "MexHotChoc", "20230213", "20221209", "20221123"))

  # single_param_df <- single_param_df %>%
  # group_by(source_colony) %>%
  # mutate(
  #   values = scale(values)[,1]  # Scale and extract the scaled values as a vector
  # ) %>%
  # ungroup()
...
# Assuming "metric" is a variable containing the name of the metric you want to map
  ...
  # reverse levls for model
  df_for_stats$QL <- df_for_stats$QR == 0

  fm.null <- lmer(values ~ 1 + zeit + (1 | day) + (1 | source_colony), data = df_for_stats)
  fm <- lmer(values ~ 1 + QL + zeit + (1 | day) + (1 | source_colony), data = df_for_stats)
