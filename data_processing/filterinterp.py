...
indices = sorted(
    [track_names.index(track) for track in formatted_list if track in track_names]
)
locations = locations[:, :, :, indices]
px_mm = 15.5
sum_of_defined_nodes = np.apply_over_axes(
    np.sum, ~np.isnan(locations[:, :, 0, :]), [0, 2]
)
velparam = 10
coordparam = 8
edgeparam = 5
...
sum_of_defined_filtered_nodes = np.apply_over_axes(
    np.sum, ~np.isnan(integfilter[:, :, 0, :]), [0, 2]
)
...
sum_of_defined_filtered_nodes = np.apply_over_axes(
    np.sum, ~np.isnan(edgefilterlocs[:, :, 0, :]), [0, 2]
)
print("PostEdgeFilter:", np.sum(sum_of_defined_filtered_nodes[:]))
spacefilterlocs = removeSharedNodes(
    edgefilterlocs, 0, 71999, shared_dist_threshold=spaceparam
)
sum_of_defined_filtered_nodes = np.apply_over_axes(
    np.sum, ~np.isnan(spacefilterlocs[:, :, 0, :]), [0, 2]
)
print("PostSpaceFilter:", np.sum(sum_of_defined_filtered_nodes[:]))
interpdlocs = fill_missing(
    spacefilterlocs[:, :, :, :], kind="linear", limit=interpparam
)
sum_of_defined_interp_nodes = np.apply_over_axes(
    np.sum, ~np.isnan(interpdlocs[:, :, 0, :]), [0, 2]
)
